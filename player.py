...
class Player:
    def __init__(self, x, y):
        ...
        self.base_speed, self.sprint_speed, self.current_speed = (
            BASE_SPEED,
            SPRINT_SPEED,
            BASE_SPEED,
        )
    ...
    def handle_input(self, walls_list, map_width, map_height):
        self.is_moving = False
        input_x = int(gl.key("d")) - int(gl.key("a"))
        input_y = int(gl.key("s")) - int(gl.key("w"))
        is_moving_keys = input_x != 0 or input_y != 0
        ...
        dx, dy = 0, 0
        if is_moving_keys:
            distance = math.hypot(input_x, input_y) or 1
            dx = (input_x / distance) * self.current_speed
            dy = (input_y / distance) * self.current_speed
            self.dir_x, self.dir_y = input_x, input_y
            self.is_moving = True

        if self.is_moving and "player_walk" in settings.SOUNDS:
            current_interval = (
                self.sprint_step_interval
                if self.current_speed > self.base_speed
                else self.walk_step_interval
            )
    ...
    def update(self, walls_list, hazard_tiles, exit_tiles, map_width, map_height):
        ...
        if self.is_moving:
            anim_speed = (
                SPRINT_ANIM_INTERVAL
                if self.current_speed > self.base_speed
                else self.anim_speed
            )
    ...
    def draw(self, cam_x, cam_y):
        ...
        if "player_sheet" in settings.TEXTURES:
            ...
            sheet_rect = pg.Rect(
                col * self.sprite_w, row * self.sprite_h, self.sprite_w, self.sprite_h
            )
            ...
            pg.display.get_surface().blit(
                scaled_player, (self.x - cam_x + shiver_x, self.y - cam_y + shiver_y)
            )
        else:
            gl.draw.rect(
                self.x - cam_x + shiver_x,
                self.y - cam_y + shiver_y,
                self.size,
                self.size,
                "green",
            )
