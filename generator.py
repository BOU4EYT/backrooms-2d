    ...
    for y in range(rows):
        ...
        for x in range(cols + 1):
            ...
            if faces_open and run_start is None:
                ...
                segments.append(
                    (run_start * tile_size, y * tile_size, x * tile_size, y * tile_size)
                )
                run_start = None
    ...
    for x in range(cols):
        ...
        for y in range(rows + 1):
            ...
            if faces_open and run_start is None:
                ...
                segments.append(
                    (x * tile_size, run_start * tile_size, x * tile_size, y * tile_size)
                )
                run_start = None
...
def generate_backrooms_level(level_id, player=None):
    ...
    while len(rooms) < target_room_count and attempts < max_attempts:
        ...
        rw, rh = random.randint(
            spec["min_size"][0], spec["max_size"][0]
        ), random.randint(spec["min_size"][1], spec["max_size"][1])
        if (
            cols - rw - MIN_ROOM_MARGIN < MIN_ROOM_MARGIN
            or rows - rh - MIN_ROOM_MARGIN < MIN_ROOM_MARGIN
        ):
            continue
        ...
        for r in rooms:
            ex, ey, ew, eh = r["rect"]
            if not (
                rx + rw + ROOM_PADDING <= ex
                or rx >= ex + ew + ROOM_PADDING
                or ry + rh + ROOM_PADDING <= ey
                or ry >= ey + eh + ROOM_PADDING
            ):
                overlaps = True
                break
        if overlaps:
    ...
    exit_room = (
        max(
            rooms[1:],
            key=lambda r: abs(r["center"][0] - scx) + abs(r["center"][1] - scy),
        )
        if len(rooms) > 1
        else rooms[0]
    )
    ...
    for y in range(rows):
        for x in range(cols):
            ...
            if cell == WALL_CELL:
                ...
                density = settings.ROOM_TYPES.get(rtype, {}).get(
                    "hazard_density", DEFAULT_HAZARD_DENSITY
                )
